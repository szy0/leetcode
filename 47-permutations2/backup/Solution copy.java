    ...
    public List<List<Integer>> permuteUnique(int[] nums) {
        ...
        if (last != prev[prev.length-1]) {
            ...
            for (List<Integer> m : prevAll) {
                all.addAll(insertToPermute(m, last));  
            } 

        } else {
            int count = 2;
            while (prev.length>=count && last == prev[prev.length-count]) count++;

            int[] arr = new int[count];
            Arrays.fill(arr, last);
            ...
            for (List<Integer> m : prevAll) {
                all.addAll(backInsertToPermute(m, arr));  
            } 
        }
